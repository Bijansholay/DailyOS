...
test('Auth: Duplicate Email Rejection', async () => {
  const email = `dupuser_${Date.now()}@example.com`;
  const passwordHash = 'hashedPassword123';

  // First registration
  const user1 = await dbEngine.createUser({ email, password_hash: passwordHash });
  assert.ok(user1.id);

  // Second registration with same email should throw error
  await assert.rejects(
    async () => {
      await dbEngine.createUser({ email, password_hash: passwordHash });
    },
    {
...
(async () => {
  try {
    ...
    if (express && authRouter && tasksRouter && eventsRouter) {
      test('HTTP API Endpoints (Express Integration)', async () => {
        ...
        try {
          ...
          const { token: tokenA } = await regResA.json();
          ...
          const { token: tokenB } = await regResB.json();
